...
const PantryOrder = () => {
  ...
  console.log("pantryoooo", pantryItems);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {allPantry.map((item, index) => (
              <div key={index} className="bg-white rounded-lg shadow-lg p-6">
                <h3 className="text-xl font-bold text-gray-800">{item.staffName}</h3>
                ...
                {item.assignedTasks && item.assignedTasks.length > 0 && (
                  <div className="mt-4">
                    ...
                    {item.assignedTasks.map((task, idx) => (
                      <div key={task._id} className={`p-4 mt-2 rounded-lg ${
                ...
                <div className="mt-4">
                  ...
                  {item.pantryItems.map((pantryItem) => (
                    <div key={pantryItem._id} className="mt-2 p-4 bg-gray-100 rounded-lg">
                      <p><strong>Item Name:</strong> {pantryItem.name}</p>
                      <p><strong>Category:</strong> {pantryItem.category}</p>
                      <p><strong>Quantity:</strong> {pantryItem.quantity} {pantryItem.unit}</p>
                      <p><strong>Supplier:</strong> {pantryItem.supplier}</p>
                      <p><strong>Storage Location:</strong> {pantryItem.storageLocation}</p>
                      <p><strong>Expiry Date:</strong> {new Date(pantryItem.expiryDate).toLocaleDateString()}</p>
                    </div>
                  ))}
                </div>
              </div>
